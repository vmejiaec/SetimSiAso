...
namespace FlexiSqlTools.Presentation.WindowsFormsApplication
{
    public partial class MainForm : Form
    {
        ...
        private void GenerateTreeAndFillDBCombo()
        {
            ...
            TreeNode tn = new TreeNode(txtDataBase.Text, GetTablesAndViewsAsTreeNode(txtDataBase.Text));
        ...
        private void btnBuild_Click(object sender, EventArgs e)
        {
            ...
            //{
                ...
                string generatedQuery = GenerateAndExecuteQuery(nodeSelect);
        ...
        private TreeNode[] GetTablesAndViewsAsTreeNode(string dbName)
        {
            ...
            if (dsSchemas != null && dsSchemas.Tables.Count > 0)
            {
                ...
                foreach (DataRow drTemp in dsSchemas.Tables[0].Rows)
                {
                    if (drTemp["Schema_Name"].ToString() == "dbo")
                        dr = drTemp;
                }
        ...
        public MainForm()
        {
            ...
            // Cambio para que no exista el parámetro @InsertedId tipo output
            insertTemplate = DataBaseHelper.GetCommentForSp() +
                 "if OBJECT_ID ('|sp_name|') is not null\r\n" +
                 "Begin\n" +
                 "    Drop Proc |sp_name|\n" +
        ...
        private string GenerateAndExecuteQuery(TreeNode databaseNode)
        {
            ...
            if (dicTables_dicColumns.Count > 0)
            {
                ...
                foreach (TreeNode sn in databaseNode.Nodes)
                    if (sn.Text == "dbo")
                        schemaNode = sn;

                //{
                    TreeNode tblNode = new TreeNode();
                    foreach (TreeNode n in schemaNode.Nodes)
                    {
                        if (n.Checked)
                            tblNode = n;
                    }
        ...
        private Collection<vw_SPGenenerator> GetTabels_Rows(string databaseName, string csvTableNames)
        {
            ...
            string selectCommand = string.Format(
                ...
                // Añadido para aumentar el control sobre las columnas calculadas
                // -------------------------------------------------------------------
            ...
            foreach (DataRow row in dsColumnsOfTable.Tables[0].Rows)
            {
                ...
                // Añadido para aumentar el control sobre las columnas calculadas
                // -------------------------------------------------------------------
                record.IsComputed = (int?)row["IsComputed"];
                // -------------------------------------------------------------------
