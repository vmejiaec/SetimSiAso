...
namespace FlexiSqlTools.Core
{
    public class DataBaseHelper
    {
        ...
        public static string GenerateQueryFromTemplate(string dbName, IEnumerable<vw_SPGenenerator> allColumnsOfTable, StoredProcedureConfig spConfig, string template)
        {
            ...
            template = template.Replace(TemplateKeywords.SP_Name, spConfig.Name.ToString());
        ...
        public static string GenerateSelectRowSp(string dbName, IEnumerable<vw_SPGenenerator> allColumnsOfTable, StoredProcedureConfig spConfig)
        {
            ...
            sss.AppendFormat("if OBJECT_ID ('{0}') is not null {1}" +
                            "Begin {1}" +
                            "Drop Proc {0} {1}" +
                            "End{1}GO{1}",
                            spConfig.Name, Environment.NewLine);
        ...
        public static string GenerateSelectAllSp(string dbName, IEnumerable<vw_SPGenenerator> allColumnsOfTable, StoredProcedureConfig spConfig)
        {
            ...
            stringBuilder.AppendFormat("if OBJECT_ID ('{0}') is not null {1}"+
                                        "Begin {1}" +
                                        "Drop Proc {0} {1}" +
                                        "End{1}GO{1}",
                                       spConfig.Name, Environment.NewLine);

            stringBuilder.AppendFormat("Create Procedure {0}",
                                       spConfig.Name);
        ...
        public static string GenerateSearchWithPagingAndSortingTotalCountSp(string dbName, IEnumerable<vw_SPGenenerator> allColumnsOfTable, StoredProcedureConfig spConfig)
        {
            ...
            sss.AppendFormat(@"{0}" +
                             "({0}" +
                             "	", Environment.NewLine);
            sss.Append(nonPrimaryKeys.Where(x => x.COLUMN_NAME.ToLower() != "lastmodificationtime" && x.COLUMN_NAME.ToLower() != "lastmodifiedby").GetUpdateParamsConColsComputadas());
            ...
            foreach (var allCol in allColumnsOfTable)
            {
                if (ll > 0) sss.Append(@",");
                sss.AppendFormat("{4}" +
                                 "                Case When @SortField = '{3}' AND @SortDirection = 'ASC' Then [{3}] End,{4}" +
                                 "                Case When @SortField = '{3}' AND @SortDirection = 'DESC' Then [{3}] End DESC", 
            ...
            foreach (var col in textColumns.Where(x => x.Constraint_Type != "PK"))
            {
                if (j > 0)
                    sss.AppendFormat("And {0}" +
                                     "            ", Environment.NewLine);
                sss.AppendFormat("(@{3} is null    Or    [{3}] like @{3}) ", dbName, SelectTABLE_SCHEMA, spConfig.Name.TableName, col.COLUMN_NAME);
                j++;
            }
            ...
            foreach (var col in nonTextColumns.Where(x => x.Constraint_Type != "PK").Where(x => x.COLUMN_NAME.ToLower() != "lastmodificationtime" && x.COLUMN_NAME.ToLower() != "lastmodifiedby"))
            {
                if (j > 0)
                    sss.AppendFormat("And {0}" +
                                     "            ", Environment.NewLine);
                sss.AppendFormat("(@{3} is null    Or    [{3}] = @{3}) ", dbName, SelectTABLE_SCHEMA, spConfig.Name.TableName, col.COLUMN_NAME);
                j++;
            }
            sss.AppendFormat("{0}" +
                             "    )AllResults{0}" +
