...
namespace FlexiSqlTools.Core
{
    public static class TemplateExtensions
    {
        ...
        {
            // Modificado para quitar las columnas calculadas
            return allColumnsOfTable.Where(x => (x.COLUMN_NAME.ToLower() != "lastmodificationtime" && x.IsComputed != 1));
        }

        //--------------------------------------------------------------
        public static IEnumerable<vw_SPGenenerator> GetInputParamsConColsComputadas(this IEnumerable<vw_SPGenenerator> allColumnsOfTable)
        {
            // Modificado para quitar las columnas calculadas
        ...
        public static StringBuilder GetUpdateColumnListString(this IEnumerable<vw_SPGenenerator> allColumnsOfTable)
        {
            ...
            foreach (var row in allColumnsOfTable)
            {
                ...
                // Modificar para quitar las columnas calculadas
                if (row.IsComputed == 1) continue;
        ...
        //------------------------------------------------------
        public static StringBuilder GetUpdateParamsConColsComputadas(this IEnumerable<vw_SPGenenerator> allColumnsOfTable)
        {
            StringBuilder sss = new StringBuilder();
            int i = 0;
            foreach (var row in allColumnsOfTable.GetInputParamsConColsComputadas())
            {
                if (i > 0)
                    sss.AppendFormat(", {0}" +
                                     "	", Environment.NewLine);
