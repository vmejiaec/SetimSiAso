...
{
    public partial class asoServicioSocio_View : SetimModulo
    {
        // Entidad base
        ...
        private IList<asoServicioSocio> ConsultaSP()
        {
        ...
        {
            asoSetimListaDetControl SetimLista = new asoSetimListaDetControl();
            var lista = SetimLista._0SelBy_asoSetimLista_Nombre("asoServicioSocio_Filtro_Campos");
        ...
        protected void btAccion_OnClick(object sender, EventArgs e)
        {
            try
            {
                ...
                _EntidadControl._5CargaSociosActivos(p_asoServicio_Id);
                Response.Redirect(Request.RawUrl, false);
                Context.ApplicationInstance.CompleteRequest();
            }
            catch (Exception exc)
            {
                Exceptions.LogException(exc);
                const string headerText = "Error";
                const string messageText = "Hay error al cargar los socios al servicio.";
                Skin.AddModuleMessage(this,
                    headerText,
                    messageText,
                    DotNetNuke.UI.Skins.Controls.ModuleMessage.ModuleMessageType.YellowWarning);
